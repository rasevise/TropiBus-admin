...
app.put(_stopsURL + '/updateStop', (req, res, next) => {
    ...
    db.query(updateStop,[stop_name ,stop_description, stop_latitude, stop_longitude, stop_id] , (err:any, result:any) => {
        if (err) {
            console.error(err); res.send('Error' + err);
        }
        db.query(asignStopToRoute, [route_id, stop_id], (err:any, result1:any) => {
        if(err) {
            console.error(err);
            res.send('Error ' + err);
        }else {
