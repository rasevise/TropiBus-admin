...
app.get(_stopsURL + '/getStopsFromRoute', function(req, res, next) {//Parameter: Route ID
...
app.put(_stopsURL + '/updateStop', function (req, res, next) {
    ...
    db.query(updateStop,[stop_name ,stop_description, stop_id] ,function(err:any, result:any) {
        if (err){ 
            console.error(err); res.send("Error" + err); 
        }
        else {
...
app.put(_stopsURL + '/updateStopOrder', function (req, res, next) {
    ...
    for (let i = 0; i < stops.length; i++) {
        db.query('UPDATE stop SET stop_order=$1 WHERE stop_id=$2',[i, stops[i].id] ,function(err:any, result:any) {
            if (err){ 
                console.error(err); res.send("Error" + err); 
            }
        });
...
app.post(_stopsURL + '/createStop', function(req, res, next) {
    ...
    db.query(createStop,[stop_name ,stop_description, stop_latitude, stop_longitude] ,function(err:any, result:any) {
        ...
        db.query(asignStopToRoute, [routeID, stopID], function(err:any, result:any){
        if(err){ 
            console.error(err); 
            res.send("Error " + err); 
        }
...
app.delete(_stopsURL + '/deleteStop', function (req, res, next) {
    ...
    db.query(deleteStop,[req.query.stop_id] ,function(err:any, result:any) {
        if (err){ 
            console.error(err); res.send("Error " + err); 
        }
        else {
