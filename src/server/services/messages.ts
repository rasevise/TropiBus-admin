...
app.post(_messagesURL + '/addMessage', (req, res, next) => {
  ...
  db.query('INSERT INTO Message(message_text, message_date, admin_id, message_title) VALUES($1, $2, $3, $4)', [req.body.messageContent,m_date,a_id,req.body.title], function(err:any, result:any) {
    if(err) {
...
app.put(_messagesURL + '/updateMessage', (req, res, next) => {
    ...
    db.query('UPDATE Message SET message_title = $2, message_text = $3  WHERE message_id = $1', [req.body.id, req.body.title, req.body.messageContent], function(err:any, result:any) {
      if (err) {
        return console.error('error running query', err);
      }
      res.send(result);
